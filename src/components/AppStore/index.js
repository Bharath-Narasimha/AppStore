...
class AppStore extends Component {
  ...
  selecttab = tabId => {
    const tabid = tabsList.find(each => each.tabId === tabId)
    this.setState({activetabId: tabid.tabId})
  }

  getappslist = () => {
    const {activetabId} = this.state
    const applist = appsList.filter(each => each.category === activetabId)
    return applist
  }
 ...
 getappslist = filteredList => { // add filteredList here
    const {activetabId} = this.state
    const applist = filteredList.filter(each => each.category === activetabId) // change appList to filteredList here
    return applist
}
  render() {
   ...
   const newlist = this.getappslist(filteredList) 
   // const newlist = searchInput === '' ? getappList : filteredList
    return (
